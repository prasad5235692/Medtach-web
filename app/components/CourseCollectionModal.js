...
function getCoursePriceLabel(course, language, content) {
...
export default function CourseCollectionModal({
  ...
  const content = getClientPageContent("courseCollectionModal", language);
  ...
  const collectionContent = isCart ? content.cart : content.wishlist;
        ...
        <div className="flex items-start justify-between border-b border-gray-100 px-6 py-5">
          <div>
            <div className="flex items-center gap-2 text-sm font-semibold text-purple-700">
              {icon}
              <span>{collectionContent.title}</span>
              <span className="rounded-full bg-purple-100 px-2 py-0.5 text-xs text-purple-700">{items.length}</span>
            </div>
            <p className="mt-1 text-sm text-gray-500">{collectionContent.description}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
                ...
                <div key={getCourseIdentifier(course)} className="flex flex-col gap-4 rounded-2xl border border-gray-100 p-4 shadow-sm sm:flex-row sm:items-center">
                  <div className="relative h-24 w-full overflow-hidden rounded-2xl bg-purple-50 sm:w-28 sm:min-w-28">
                    {course?.thumbnail ? (
                      <Image
                        ...
                        alt={course.title || course.name || content.defaults.imageAlt}
                        fill
                        sizes="112px"
                        unoptimized
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-semibold uppercase tracking-wide text-purple-500">
                      {course?.category || (isCart ? content.defaults.cartCategory : content.defaults.favoritesCategory)}
                    </p>
                    <h3 className="mt-1 line-clamp-2 text-sm font-bold text-gray-900">
                      {course?.title || course?.name || content.defaults.untitledCourse}
                    </h3>
                    <div className="mt-2 flex flex-wrap gap-2 text-xs text-gray-500">
                      <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-1">
                        <Clock size={12} />
                        {getCourseDurationLabel(course, content)}
                      </span>
                      <span className="inline-flex items-center rounded-full bg-purple-50 px-2.5 py-1 font-semibold text-purple-700">
                        {getCoursePriceLabel(course, language, content)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex shrink-0 items-center gap-2 sm:flex-col sm:items-end">
                    ...
                    >
                      <Trash2 size={13} />
                      {isCart ? content.removeLabel : content.unsaveLabel}
                    </button>
                    <Link
