...
import { getCohortLearningContent } from "@/business/what-we-do/cohort-learning/data/content";
...
export async function generateMetadata() {
  ...
  const content = getCohortLearningContent(locale);
    ...
    <div className="bg-white">
      <section className="bg-linear-to-br from-[#0d0422] via-[#1a0535] to-[#0f172a] py-24 text-white">
        <div className="mx-auto max-w-4xl px-6 text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-purple-400/30 bg-purple-600/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-purple-300">
            <Users size={13} /> {content.hero.badge}
          </span>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-6xl px-6">
          <div className="text-center">
            <p className="text-xs font-semibold uppercase tracking-widest text-purple-700">{content.cohortTypesSection.label}</p>
            <h2 className="mt-3 text-3xl font-extrabold text-gray-900 md:text-4xl">{content.cohortTypesSection.title}</h2>
          </div>
          <div className="mt-12 grid gap-6 md:grid-cols-2">
            {content.cohortTypesSection.items.map((item) => (
              <div key={item.id} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition hover:border-purple-200 hover:shadow-md">
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid items-center gap-12 md:grid-cols-2">
            <div>
              ...
              <p className="mt-4 text-base text-gray-600">{content.benefitsSection.description}</p>
            ...
            <div className="flex flex-col gap-5">
              {content.benefitsSection.featureCards.map((card) => {
                const Icon = featureIcons[card.icon] ?? Calendar;

                return (
                  <div key={card.id} className="flex items-start gap-4 rounded-2xl bg-white p-5 shadow-sm">
      ...
      <section className="py-16">
        <div className="mx-auto max-w-3xl px-6 text-center">
          <blockquote className="text-xl font-medium italic text-gray-700">{content.testimonialSection.quote}</blockquote>
          <p className="mt-4 text-sm font-semibold text-purple-800">{content.testimonialSection.name}</p>
          <p className="text-xs text-gray-500">{content.testimonialSection.role}</p>
        </div>
      </section>
