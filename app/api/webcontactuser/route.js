...
const BACKEND_ROUTE_BASE = "/webcontactuser";
...
function normalizeBackendBaseUrl(baseUrl) {
  return String(baseUrl || "")
    .trim()
    .replace(/\/+$/, "")
    .replace(/\/(?:wedstudentuser|webcontactuser)$/i, "");
}
...
export async function POST(request) {
  ...
  for (const backendBaseUrl of getBackendBaseUrls()) {
    try {
      ...
      lastFailure = {
        status: 502,
        payload: createErrorPayload("Unable to reach the contact API"),
      };
      console.warn("[webcontactuser proxy] backend request failed", {
        backendBaseUrl,
  ...
  return NextResponse.json(
    lastFailure?.payload || createErrorPayload("Unable to reach the contact API"),
    {
